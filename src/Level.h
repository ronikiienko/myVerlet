...
class Level : public BaseLevel {
    ...
    Shooter shooter{Vector2::cart(200, 200), 2, 1, 4, atomWorld, 1, explosionHandler};
...
public:
    ...
    void onInit() override {
        ...
        atomWorld.addObject(Player{inputHandler, camera, shooter, atomWorld}, Vector2::cart(100, 100));
        ...
        inputHandler.addEventListener(sf::Event::MouseButtonPressed, [&](sf::Event &event) {
            if (event.mouseButton.button == sf::Mouse::Right) {
                explosionHandler.launch(
                        camera.screenPosToWorldPos(Vector2::cart(static_cast<float>(event.mouseButton.x),
